...
class _NowPlayingScreenState extends ConsumerState<NowPlayingScreen> {
  ...
  // ★ BugFix: Color は非 null 型なので「_dominantColor == null」チェックは
  //   常に false になる。代わりに「最後に色を抽出した曲ID」でガードする。
  ...
  Future<void> _maybeExtractDominantColor(String songId, Uint8List? imageBytes) async {
            ...
            child: Column(
              ...
              children: [
                ...
                Slider(
                  ...
                  onChanged: (value) => setModalState(() => _playbackSpeed = value),
                  onChangeEnd: (value) {
            ...
            child: SafeArea(
              child: Column(
                children: [
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Consumer(
                            builder: (_, ref, __) {
                              final title = ref.watch(audioPlayerProvider).currentSong?.title ?? '';
                              return Text(
                                title,
                                style: const TextStyle(
                        ...
                        IconButton(
                          icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white38, size: 28),
                          onPressed: () => Navigator.pop(context),
                        ),
                      ],
                    ),
                  ...
                  // ★ 歌詞スクロールビュー本体
                  const Expanded(child: _FullScreenLyricView()),
                ],
              ),
            ),
          );
  ...
  void _showAudioRoutePicker(BuildContext context) async {
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('出力先の変更に失敗しました: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  // ──────────────────────────────────────────────────────────
  // ★ インライン歌詞カード（再生画面下部の「Spotify カード」）
  // ──────────────────────────────────────────────────────────

  Widget _buildInlineLyricsCard(List<LyricLine> lyrics) {
    // dominantColor に黒を重ねた「一段暗い」カード背景
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      if (index < ci - 1 || index > ci + 2) return const SizedBox.shrink();
  ...
  Widget build(BuildContext context) {
    ...
    if (currentSong == null) {
      return Scaffold(
        ...
        body: const Center(
          child: Text('Playback Screen', style: TextStyle(color: AppColors.textPrimary)),
        ),
      );
    }
                          ...
                          Container(
                            ...
                            child: ClipRRect(
                              ...
                              child: currentSong.albumArt != null
                                  ? Image.memory(currentSong.albumArt!, fit: BoxFit.cover)
                                  : Container(
                              ...
                              children: [
                                ...
                                Text(
                                  () {
                                    ...
                                    return (a.isEmpty || a.toLowerCase().contains('unknown'))
                                        ? 'Mrs. GREEN APPLE'
                                        : a;
                                  }(),
                                  style: TextStyle(
                            ...
                            children: [
                              ...
                              IconButton(
                                icon: Icon(Icons.repeat,
                                    color: playerState.repeatMode != PlaylistMode.off
                                        ? Colors.white
                                        : Colors.white.withValues(alpha: 0.5)),
...
// ============================================================
// ★ フルスクリーン歌詞ビュー（Spotify カラオケ UI）
// ============================================================

class _FullScreenLyricView extends ConsumerStatefulWidget {
  const _FullScreenLyricView();

  @override
  ConsumerState<_FullScreenLyricView> createState() => _FullScreenLyricViewState();
}

class _FullScreenLyricViewState extends ConsumerState<_FullScreenLyricView> {
  ...
  void _scrollTo(int index) {
    ...
    final target = topPad + index * _lineH - vp / 2 + _lineH / 2;
    _sc.animateTo(
      target.clamp(_sc.position.minScrollExtent, _sc.position.maxScrollExtent),
  ...
  Widget build(BuildContext context) {
    ...
    if (currentIndex != _lastIndex && currentIndex >= 0) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollTo(currentIndex));
    }
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('歌詞がありません', style: TextStyle(color: Colors.white38, fontSize: 16)),
          ],
        ),
      );
    }
